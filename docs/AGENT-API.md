...
## 2b. Campaign Strategy & Approval Flow

### Create Campaign with Strategy Brief
```
POST /agent/v1/campaigns/create
```
Used by Hermes to propose new campaigns. Humans review the strategy_brief in the UI and approve before contacts are pushed.

**Body:**
```json
{
  "name": "CE-SL: Electrical Companies 001 - US - ARKODE",
  "description": "First cold email campaign targeting electrical contractors in US",
  "strategy_brief": "We are targeting electrical contractors with 25-200 employees in the US. These companies typically struggle with manual scheduling, job costing, and invoicing. Our angle is operational efficiency: showing how Odoo replaces 4-5 tools they currently juggle. Initial outreach focuses on pain points (missed appointments, revenue leakage from unbilled change orders). Follow-ups pivot to ROI and case studies.",
  "target_vertical": "Electrical Contractors",
  "target_icp": "Owners and GMs of electrical companies with 25-200 employees, $2M-$50M revenue",
  "hypothesis": "Electrical contractors respond better to cost-savings messaging (reduce overhead by 30%) than growth messaging (scale your business) based on HVAC campaign results",
  "workspace": "US",
  "status": "Draft"
}
```
...
```json
{
  ...
  "status": "Draft",
  "created_by": "hermes",
  "message": "Campaign created. Awaiting human approval before contacts can be pushed.",
...
```
PUT /agent/v1/campaigns/{campaign_id}/strategy
```

**Body:**
```json
{
  "strategy_brief": "Updated strategy after reviewing HVAC results...",
  "hypothesis": "Updated hypothesis based on new data"
}
```

### What Humans See in the UI
When a campaign is created by Hermes (created_by = "hermes"):
- **"Agent" badge** appears on the campaign card
- **"Needs Approval" tag** shows until approved
- **Expanded view** shows: Target Vertical, ICP, Strategy Brief, Hypothesis
- **"Approve Campaign" button** for human to approve
- Once approved, contacts can be pushed to ReachInbox

### Campaign Lifecycle
```
Hermes creates campaign (status=Draft, created_by=hermes)
  ↓
Human reviews strategy brief in Deduply UI
  ↓
Human clicks "Approve Campaign" (sets approved_by, approved_at)
  ↓
Hermes runs bulk pipeline, stages contacts (POST /agent/v1/leadgen/bulk-run)
  ↓
Human reviews contacts in Pipeline page
  ↓
Hermes pushes approved contacts with dedup (POST /agent/v1/campaigns/{id}/push-with-dedup)
```

## 2c. Dedup-Before-Push Flow

### Push Contacts with Deduplication
```
POST /agent/v1/campaigns/{campaign_id}/push-with-dedup
```
The complete dedup + push flow. Every contact goes through the master database before reaching ReachInbox.

**Body:**
```json
{
  "job_id": "abc123",
  "ri_campaign_id": 103996,
  "workspace": "US"
}
```
...
1. Takes staged contacts (from pipeline job) or specific contact IDs
2. For each contact, checks master DB by email (case-insensitive)
3. **If duplicate found:** merges missing fields (fills in blanks on existing record), links to campaign
4. **If new:** inserts into master contacts DB with workspace tag
5. All valid contacts pushed to ReachInbox campaign
6. Contact status updated to "Pushed to RI"
...
```json
{
  ...
  "stats": {
    "total_input": 50,
    "new_contacts": 35,
    "merged_duplicates": 12,
    "skipped_no_email": 3,
    "pushed_to_ri": 47,
    "push_failed": 0
  },
  "message": "35 new + 12 merged = 47 pushed to ReachInbox",
  "next_action": "monitor_campaign"
}
```

### Why Dedup Matters
- **47,897 contacts** already in the master DB (38K US + 9.8K MX)
- Without dedup, same person gets emailed from multiple campaigns
- Dedup preserves contact history: if someone replied to HVAC campaign, they won't get re-added to Roofing campaign
- Merge fills in gaps: if a contact had no title before, and the new pipeline found their title, it gets updated
...
*Last synced: 2026-03-04 01:00 EST*
